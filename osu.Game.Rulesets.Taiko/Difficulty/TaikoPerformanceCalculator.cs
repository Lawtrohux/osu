...
namespace osu.Game.Rulesets.Taiko.Difficulty
{
    public class TaikoPerformanceCalculator : PerformanceCalculator
    {
        ...
        {
            double baseValue = Math.Pow(5 * Math.Max(1, attributes.StarRating / 0.115) - 4, 2.25) / 1150;
            double difficultyValue = baseValue * (1 + 0.01 * Math.Pow(attributes.StarRating - 5.5, 2)) * (1 + 0.05 * Math.Pow(attributes.StarRating - 10, 2));
